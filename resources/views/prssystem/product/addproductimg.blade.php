...
@if($imgObj->status==1)
<div class="col-md-3" style="backgroud-color:green; min-height: 350px">
  <div class="thumbnail">
    <a href="{{config('global.PRODUCTS_NEW_STORAGE_DIR')
    .DIRECTORY_SEPARATOR.$imgObj->user_product_id
    .DIRECTORY_SEPARATOR
    .config('global.PRODUCT_IMG_WIDTH').'X'.config('global.PRODUCT_IMG_HEIGHT')
    .DIRECTORY_SEPARATOR
    .$imgObj->image_name}}" target="_blank">
      <img src="{{config('global.PRODUCTS_NEW_STORAGE_DIR')
    .DIRECTORY_SEPARATOR.$sellerId
    .DIRECTORY_SEPARATOR
    .config('global.PRODUCT_IMG_WIDTH').'X'.config('global.PRODUCT_IMG_HEIGHT')
    .DIRECTORY_SEPARATOR
    .$imgObj->image_name}}" alt="Image" style="width:100%;min-height: 300px">
  
      <div class="caption">
          <div class="btn-group btn-group-justified">
            @if($imgObj->is_default==0)
            <a href="{{route('setasdefault',['prod_id'=>$sellerId,'id'=>$imgObj->id])}}" class="btn btn-warning" title="Set Default Image on Page">Set As Default</a>
            @endif
...
<div class="col-md-3" style="backgroud-color:green; ">
  <div class="thumbnail">
    <a href="{{config('global.PRODUCTS_NEW_STORAGE_DIR').DIRECTORY_SEPARATOR.$imgObj->user_product_id.DIRECTORY_SEPARATOR.$imgObj->image_name}}" target="_blank">
      <img src="{{config('global.PRODUCTS_NEW_STORAGE_DIR').DIRECTORY_SEPARATOR.$imgObj->user_product_id.DIRECTORY_SEPARATOR.$imgObj->image_name}}" alt="Image" style="width:100%;min-height: 300px">
