...
class PaymentController extends Master
{
	...
	public function choosepayment(Request $request){
		//dd($request->get('shippingAddress'));
		...
		try{
			if($request->get('shippingAddress')!=''){
				$shippingId = decrypt($request->get('shippingAddress')); 
				$DeliveryAddress = DeliveryAddress::with('State','City')->where('id','=',$shippingId)->first()->toArray();
				//dd($DeliveryAddress);
			}else{
				abort(404);
			}
			...
			return view(Master::loadFrontTheme('firezyshop.Payment.index'),
			array(
				'totalAmount'=>$total,
				'count'=>$count,
				'quantity'=>$quantity,
