...
class CartController extends Master
{
    ...
    public function addToCart(Request $request){
        ...
        if($userProductId>0){
            ...
            $imgUrlDefaultThumbnail = config('global.PRODUCTS_NEW_STORAGE_DIR').DIRECTORY_SEPARATOR.$productDetails['seller_id'].DIRECTORY_SEPARATOR.config('global.PRODUCT_THUMB_IMG_WIDTH').'X'.config('global.PRODUCT_THUMB_IMG_HEIGHT').DIRECTORY_SEPARATOR.$productDetails['default_thumbnail'];
	...
	public function checkoutinit(Request $request){
		...
		return view(Master::loadFrontTheme('firezyshop.CheckOut.index'),
    ...
    public function index(Request $request){
		...
		return view(Master::loadFrontTheme('firezyshop.Cart.index'),array(
			'cartItem'=>$itemList,
			'count'=>$cartCollections->count(),
			'quantity'=>\Cart::getTotalQuantity(),
			'subTotal'=>\Cart::session($userId)->getSubTotal(),
